...
class Activity extends CRMEntity {
	...
	function save_module($module)
	{
      //echo "<pre>";print_r($this->column_fields);die;
        ...
        if(($_REQUEST['sourceModule'] == "Contacts" || $this->column_fields['contact_id'] != "" || $_REQUEST['sourceRecord'] != "")) // create new lead
            $this->createUpdateLead();
	}	

	function createUpdateLead() {
        global $adb, $current_user;
        $date_format = $current_user->date_format;
        $assignedid = $_REQUEST['assigned_user_id'];
		if($_REQUEST['sourceRecord'] != '')
        $parent_id = $_REQUEST['sourceRecord'];
		else
		$parent_id = $this->column_fields['contact_id'];
		
        $arr_length = sizeof($_REQUEST);
        $loop_length = floor($arr_length/7);
        for($i = 1; $i <= $loop_length; $i++) {		// loop
            $product = $_REQUEST['product'.$i];
            $expected_revenue = $_REQUEST['expected_revenue'.$i];
            $expected_closure_date = $_REQUEST['expected_closure_date'.$i];
            $expected_closure_date = DateTimeField::__convertToDBFormat($expected_closure_date, $date_format);
            $lead_stage = $_REQUEST['lead_stage'.$i];
            $remarks = $_REQUEST['remarks'.$i];
            $leadid = $_REQUEST['leadid'.$i];
            if($product != "" && $expected_revenue != "" && $expected_closure_date != "" && $lead_stage != "") {
                if($leadid != ""){
                    $leadvalues = array(
                        'cf_907'=>$product,
                        'cf_909'=>$expected_revenue,
                        'cf_911'=>$expected_closure_date,
                        'cf_913'=>$lead_stage,
                        'cf_915'=>$remarks,
                    );
                    $this->updateLead($leadid, $parent_id, $product, $expected_revenue, $expected_closure_date, $lead_stage, $remarks, $leadvalues);
                }
                else {
                    ...
                    $query = "INSERT INTO vtiger_crmentity (crmid,smcreatorid,smownerid,setype,createdtime,modifiedtime,label)
                              VALUES (?,?,?,?,?,?,?)";
                    $adb->pquery($query, array($crmid, $createrid, $assignedid,"Leads", $currentdatetime, $currentdatetime,$leadno));

                    $adb->query("INSERT INTO vtiger_leaddetails(leadid, lead_no) VALUES (".$crmid.", '".$leadno."')");
                    $adb->query("INSERT INTO vtiger_leadsubdetails(leadsubscriptionid) VALUES (".$crmid.")");
                    $adb->query("INSERT INTO vtiger_leadscf(leadid, cf_907, cf_909, cf_911, cf_913, cf_915)
                                VALUES(".$crmid.", '".$product."', '".$expected_revenue."', '".$expected_closure_date."', '".$lead_stage."', '".$remarks."')");
                    $adb->query("INSERT INTO vtiger_leadaddress (leadaddressid) VALUE (".$crmid.")");

                    $sql = "insert into vtiger_crmentityrel values (?,?,?,?)";
                    $adb->pquery($sql, array($this->id,'Events',$crmid,'Leads'));

                    $sql = "insert into vtiger_crmentityrel values (?,?,?,?)";
                    $adb->pquery($sql, array($parent_id,'Contacts',$crmid,'Leads'));
    ...
    function updateLead($leadid, $parent_id, $product, $expected_revenue, $expected_closure_date, $lead_stage, $remarks, $leadvalues) {
        global $adb, $current_user;
        if(isset($_REQUEST['currentid']) && $_REQUEST['currentid'] != "") {
            $sql = "insert into vtiger_crmentityrel values (?,?,?,?)";
            $adb->pquery($sql, array($this->id,'Events',$leadid,'Leads'));
        }

        $lead_qry = $adb->query("SELECT cf_907, cf_909, cf_911, cf_913, cf_915 FROM vtiger_leaddetails
                            INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_leaddetails.leadid
                            INNER JOIN vtiger_leadscf ON vtiger_leadscf.leadid = vtiger_leaddetails.leadid
                            WHERE vtiger_crmentity.deleted = 0 AND vtiger_leaddetails.leadid = $leadid ");
        if($adb->num_rows($lead_qry) > 0) {
            $leadrow = $adb->fetchByAssoc($lead_qry);
            $counter = 0;
            foreach($leadvalues as $key=>$row) {
                if(vtlib_purify($leadrow[$key]) != $row)	{
                    if($counter == 0) {
                        $adb->query("UPDATE vtiger_leadscf INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_leadscf.leadid
                    SET cf_907 = '$product', cf_909 = $expected_revenue, cf_911 = '$expected_closure_date', cf_913 = '$lead_stage',
                        cf_915 = '$remarks', modifiedby = ".$current_user->id.", modifiedtime = '".date('Y-m-d H:i:s')."'
                        where leadid = $leadid ");

                        $thisid = $adb->getUniqueId('vtiger_modtracker_basic');
                        $adb->pquery('INSERT INTO vtiger_modtracker_basic(id, crmid, module, whodid, changedon, status)
					                  VALUES(?,?,?,?,?,?)', Array($thisid, $leadid, 'Leads',$current_user->id, date('Y-m-d H:i:s',time()), 0));
                    ...
                    $adb->pquery('INSERT INTO vtiger_modtracker_detail(id,fieldname,prevalue, postvalue) VALUES(?,?,?,?)',
                        Array($thisid, $key, $leadrow[$key], $row));
                    $counter++;
                }
            }
        }
