...
function getActivityDetails($description,$user_id,$from='')
{
	...
	//echo "<pre>"; print_r($_POST);die("ar");
	$list = 'Dear '.$name.',';
	$list .= '<br><br>'.$msg.' '.$reply;
	$list .= '<br><br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'.$mod_strings["LBL_SUBJECT"].' : '.$description['subject'];
	$list .= '<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'.$mod_strings["LBL_ACTIVITYTYPE"].' : '.$activitytype;	
...
function sendInvitation($inviteesid,$mode,$subject,$desc)
{
	...
	$sourceRecord = $_POST['sourceRecord'];
	$clientname = "SELECT cf_927 FROM vtiger_contactscf WHERE contactid = ".$sourceRecord ;
	$query = $adb->query($clientname);
	//echo "<pre>"; print_r($clientname); die();
	...
	$subject = $invites.' : '.$activity_type.' with '.$clientname;
	$record = $focus->id;
