...
// Change by Raghveder Singh on 02052014
    var $search_fields = Array(
		 'Name of Client' => Array('vtiger_contactscf'=>'cf_927'),
//Add by Raghvender Singh on 03052014
        'Key Contact Person' => Array('contactscf'=>'cf_815'),
//End Add by Raghvender Singh on 03052014
        'Branch' => Array('contactscf'=>'user_branch'),

        'Account Manager'=>Array('vtiger_crmentity'=>'assigned_user_id')
    ...
    var $search_fields_name = Array(
        ...
        'Name of Client' => 'cf_927',
//Add by Raghvender Singh on 03052014
        'Key Contact Person' => 'cf_815',
//End Add by Raghvender Singh on 03052014		

        'Branch' => 'user_branch',

        'Account Manager'=>'assigned_user_id'
    );
	...
	var $def_basicsearch_col = 'cf_927';
  ...
  $log->debug("Exiting get_contactsforol method ...");
	...
	*/
...

	{
		...
		if(isset($_REQUEST) && $_REQUEST['record'] == ''){
			$Contact_Person = $_REQUEST['cf_815'];	
			$key_Contact_Person = $_REQUEST['cf_819'];	
			$Role = $_REQUEST['cf_821'];	
			$Mobile_No = $_REQUEST['cf_823'];	
			$Email = $_REQUEST['cf_827'];	
			$full_Address = $_REQUEST['cf_829'];	
			$City = $_REQUEST['cf_835'];	
			$country = $_REQUEST['cf_831'];	
			$Postal_Code = $_REQUEST['cf_837'];	
			$Phone = $_REQUEST['cf_825'];	
			$Do_Not_Call  = $_REQUEST['cf_839'];	
			$Do_Not_Email  = $_REQUEST['cf_841'];	
			$Do_Not_SMS  = $_REQUEST['cf_843'];	
			$Not_Contactable_Phone  = $_REQUEST['cf_845'];	
			$Not_Contactable_Email = $_REQUEST['cf_847'];	

			$smownerid = $_REQUEST['assigned_user_id'];	
			$currentdatetime = date('Y-m-d H:i:s');
			$crmid = $crmid = $adb->getUniqueID("vtiger_crmentity");
			
			if($key_Contact_Person == 'on')
			$key_Contact_Person = 1;
			else
			$key_Contact_Person = 0;
			...
			if($Not_Contactable_Phone == 'on')
			$Not_Contactable_Phone = 1;
			else
			$Not_Contactable_Phone = 0;
			
			if($Not_Contactable_Email == 'on')
			$Not_Contactable_Email = 1;
			else
			$Not_Contactable_Email = 0;
         ...
         values('".$crmid."',1,'".$smownerid."','0','Products','','".$currentdatetime."','".$currentdatetime."','NULL','NULL','0','1','0')");
			
			$adb->query("INSERT into vtiger_products (productid,productname) values(".$crmid.",'".$Contact_Person."')");	
			
			$adb->query("INSERT into vtiger_productcf (productid, key_contact_person,contact_user_role ,mobile ,email ,full_address ,city ,country ,postal_code ,phone , do_not_call,do_not_email,do_not_sms,not_contactable_phone,not_contactable_email) values('".$crmid."','".$key_Contact_Person."','".$Role."','".$Mobile_No."','".$Email."','".$full_Address."','".$City."','".$country."','".$Postal_Code."','".$Phone."','".$Do_Not_Call."','".$Do_Not_Email."','".$Do_Not_SMS."','".$Not_Contactable_Phone."','".$Not_Contactable_Email."')");
			
			$adb->query("INSERT into vtiger_seproductsrel(crmid,productid,setype) values(".$_REQUEST['currentid'].",".$crmid.",'".$_REQUEST['module']."')");	
