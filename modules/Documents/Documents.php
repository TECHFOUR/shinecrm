...
class Documents extends CRMEntity {
	...
	function save_module($module)
	{
		...
		if($filelocationtype == 'I') {
			$newfilename = $filename_fieldname;
			$this->insertIntoAttachment($this->id,'Documents',$newfilename);
		}else{
		...
		// Start to upload multiple file ***************
if(isset($_REQUEST['sourceRecord']) && $_REQUEST['sourceRecord'] != "" && $_REQUEST['sourceModule'] == "Potentials") {	
	for($i=0; $i<=13; $i++) {
		$delete = $_REQUEST['deleted'.$i];
		$title = $_REQUEST['title_'.$i];
		$note = mysql_real_escape_string($_REQUEST['note_'.$i]);
		$newfilename = 'file_'.$i;
		$_REQUEST['notes_title'] = $note; 
		$_REQUEST['filename_hidden'] = $_FILES[$newfilename]['name'];
		$_REQUEST['notecontent'] = $note;
		$filename = $_FILES[$newfilename]['name'];
		$filetype = $_FILES[$newfilename]['type'];
		$filesize = $_FILES[$newfilename]['size'];
		if($delete == 0 && $title != '' && $filename != '') {
			$createrid = $current_user->id;
			$currentdatetime = date("Y-m-d H:i:s");	
			if($_REQUEST['assigntype'] == 'U')
				$assignedid = $_REQUEST['assigned_user_id'];
			else 
				$assignedid = $createrid; // Modified by jitendra on 30may13
          ...
          values(".$crmid.",".$createrid.",".$assignedid.",'0','Documents',' ','".$currentdatetime."','".$currentdatetime."','NULL','NULL','0','1','0')");
		  
		  $querynum = $adb->query("select prefix, cur_id from vtiger_modentity_num where semodule='Documents' and active = 1");
		  $resultnum = $adb->fetch_array($querynum);
		  $prefix = $resultnum['prefix'];
		  $cur_id = $resultnum['cur_id'];
		  $docno = $prefix.$cur_id; 
		  $next_curr_id = $cur_id + 1;
		  $adb->query("update vtiger_modentity_num set cur_id = ".$next_curr_id." where semodule='Documents' and active = 1");
		  
		  $adb->query("INSERT into vtiger_notes (notesid, note_no, title, filename, notecontent, folderid, filetype, filelocationtype, filestatus, filesize) values(".$crmid.", '".$docno."', '".$title."', '".$filename."', '".$note."', 1, '".$filetype."', 'I', 1, '".$filesize."')"); 
		  $adb->query("INSERT into vtiger_senotesrel(crmid, notesid) values(".$_REQUEST['sourceRecord'].", ".$crmid.")"); 
		  
			$errCode=$_FILES[$newfilename]['error'];
			if($errCode == 0){
				foreach($_FILES[$newfilename] as $fileindex => $files)
				{
					if($files['name'] != '' && $files['size'] > 0){
						$filename = $_FILES[$newfilename]['name'];
						$filename = from_html(preg_replace('/\s+/', '_', $filename));
			...
			$this->insertIntoAttachment($crmid,'Documents',$newfilename);
		}
	}
}
// End to upload multiple file ***************	
	...
	*/
	function insertIntoAttachment($id,$module,$newfilename)
	{
		global $log, $adb;
		$counter = 0;
