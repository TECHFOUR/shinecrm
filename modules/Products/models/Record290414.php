...
class Products_Record_Model extends Vtiger_Record_Model {
    ...
    public function checkDuplicate() {
        ...
        $record = $this->getId();
        $error_message = "";
        $query_more_contact = "SELECT cf_927 FROM vtiger_productcf
        ...
        $query_more_contact = "SELECT cf_927 FROM vtiger_productcf
                        ...
                        INNER JOIN vtiger_seproductsrel as rel ON (rel.productid = vtiger_productcf.productid AND rel.setype= 'Contacts' )
                        INNER JOIN vtiger_contactscf ON vtiger_contactscf.contactid = rel.crmid
                        WHERE setype = ? AND email = ? AND deleted = 0";
        ...
        if ($db->num_rows($result_more_contact) > 0) {
            $res = $db->fetch_array($result_more_contact);
            $client_name = $res['cf_927'];
            $error_message = "Email Id already exist in the system of this $client_name";
        }
        ...
        $query_more_contact = "SELECT cf_927 FROM vtiger_productcf
                        ...
                        INNER JOIN vtiger_seproductsrel as rel ON (rel.productid = vtiger_productcf.productid AND rel.setype= 'Contacts' )
                        INNER JOIN vtiger_contactscf ON vtiger_contactscf.contactid = rel.crmid
                        WHERE setype = ? AND mobile = ? AND email = ? AND deleted = 0";
        ...
        if ($db->num_rows($result_more_contact) > 0) {
            $res = $db->fetch_array($result_more_contact);
            $client_name = $res['cf_927'];
            $error_message = "Mobile No. and Email Id already exist in the system of this $client_name";
        }
