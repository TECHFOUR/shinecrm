...
class Products_Record_Model extends Vtiger_Record_Model {
    ...
    public function checkDuplicate() {
        ...
        $mobile = $_REQUEST['accountname'];
        $email = $_REQUEST['email'];
        $key_contact = $_REQUEST['key_contact'];
        $record = $this->getId();
        $error_message = "";

        if($record && $key_contact == 'true') {
            $pot_qry = $db->query("SELECT crmid FROM vtiger_productcf
                                            INNER JOIN vtiger_seproductsrel ON (vtiger_seproductsrel.productid = vtiger_productcf.productid AND vtiger_seproductsrel.setype = 'Contacts')
                                            WHERE vtiger_productcf.productid = $record ");
            if($db->num_rows($pot_qry) > 0) {
                $row = $db->fetchByAssoc($pot_qry);
                $potentialid = $row['crmid'];
            }

            $query_more_key_contact = "SELECT key_contact_person FROM vtiger_productcf
                            ...
                            WHERE vtiger_crmentity.setype = ? AND rel.crmid = ? AND key_contact_person = ? AND vtiger_crmentity.deleted = 0";
            $params_more_key_contact = array('Products', $potentialid, 1);
            if($record) {
                $query_more_key_contact .= " AND vtiger_crmentity.crmid != ?";
                array_push($params_more_key_contact, $record);
            }
            $result_more_key_contact = $db->pquery($query_more_key_contact, $params_more_key_contact);
            if ($db->num_rows($result_more_key_contact) > 0) {
                $error_message = "There is already Key Contact Person";
            }
        }
        ...
        if($record) {
            $query_more_contact .= " AND vtiger_crmentity.crmid != ?";
            array_push($params_more_contact, $record);
        }
