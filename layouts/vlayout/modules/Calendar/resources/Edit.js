...
////////////////////////Start Code For validation done by JITENDRA  Date - 24-03-2014 ////////
		...
		registerEventActivityTypeChangeEvent : function(container){
			...
			var activity_type = container.find('select[name="activitytype"]').val();
			
			if(activity_type == 'Meeting' && selectedOptiont == 'Held'){				
			...
			var activity_type = container.find('select[name="eventstatus"]').val();
			
			if(activity_type == 'Held' && selectedOptiont == 'Meeting'){				
...
////////////////////////End Code For validation done by JITENDRA  Date - 24-03-2014 ////////
		...
		/*Start code to hide Related Lead in Activity module by jitendra singh 30 April 2014*/
		
		container.find('select[name="cf_1396"]').on('change',function(e){
			
			var ActivityStatusEventdivlead = container.find('.related_lead_block');															

			var selectedOptiont = jQuery(e.currentTarget).val();//

			if(selectedOptiont == 'Service'){				

			   ActivityStatusEventdivlead.hide();

			  	}else{

			    ActivityStatusEventdivlead.show();

			     }

			});
		
		/*End code to hide Related Lead in Activity module by jitendra singh 30 April 2014*/
