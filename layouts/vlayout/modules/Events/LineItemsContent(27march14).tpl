...
<td xmlns="http://www.w3.org/1999/html">
    ...
    <td>
        ...
        {foreach item=PICKLIST_VALUE key=PICKLIST_NAME from=$LEAD_STAGE}
            <option value="{$PICKLIST_VALUE}" >{$PICKLIST_VALUE}</option>
        {/foreach}
        </select>
    </td>
{*<td>
...
<td>
    ...
    <input id="{$expected_revenue}" name="{$expected_revenue}" value="{$data.expected_revenue}" type="text"
           placeholder="Expected Revenue" style="width:128px" maxlength="15" title="No of ROs" class="qty smallInputBox"
...
<td>
    ...
    <input class="date field" type="Text" id="{$expected_closure_date}" value="{$data.expected_closure_date}"
           ...
           size="25"  onclick="javascript:NewCssCal(this.id)" data-validation-engine="validate[required,funcCall[Vtiger_Base_Validator_Js.invokeValidation]]"/>
</td>
