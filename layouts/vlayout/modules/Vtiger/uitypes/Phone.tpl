...
{if $MODULE eq 'Products' && $FIELD_MODEL->getFieldName() eq 'mobile'}
<input id="{$MODULE}_editView_fieldName_{$FIELD_MODEL->get('name')}" type="text"  onkeyup="ValidateCustomFields();" 
       maxlength="10" class="input-large" data-validation-engine="validate[{if $FIELD_MODEL->isMandatory() eq true} required,{/if}funcCall[Vtiger_Base_Validator_Js.invokeValidation]]" name="{$FIELD_MODEL->getFieldName()}"
