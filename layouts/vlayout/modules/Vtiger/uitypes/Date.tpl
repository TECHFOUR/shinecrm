...
{*<!-- Start code to validate Expected Closure Date by jitendra singh on 22 march 2014 -->*}   
 {if $MODULE eq 'Leads' && ($FIELD_MODEL->get("name") eq "cf_911") }
 <div class="input-append row-fluid">
	<div class="span10 row-fluid date">
		{assign var=FIELD_NAME value=$FIELD_MODEL->get('name')}
		<input onchange="Closure_Date_Validate(this.value,this.id,'{$dateFormat}');" id="{$MODULE}_editView_fieldName_{$FIELD_NAME}" type="text" class="span9 dateField" name="{$FIELD_MODEL->getFieldName()}" data-date-format="{$dateFormat}"
			type="text" value="{if $FIELD_MODEL->getEditViewDisplayValue($FIELD_MODEL->get('fieldvalue')) eq ''}{if $dateFormat eq 'yyyy-mm-dd'}{$Expected_Closure_Date}{/if}{if $dateFormat eq 'dd-mm-yyyy'}{$Expected_Closure_Date}{/if}{if $dateFormat eq 'mm-dd-yyyy'}{$Expected_Closure_Date}{/if}{else}
