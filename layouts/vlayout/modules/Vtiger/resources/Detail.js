...
function showSalesapprovalDatabase(str)
{
		...
		xmlhttp.onreadystatechange=function()
		  ...
		  if (xmlhttp.readyState==4 && xmlhttp.status==200)
			...
			error = '';
			if(arr[0].trim() == 'no'){
				error += 'Please update "Post Call Updates"\n';
				}
			if(arr[1].trim() == 'no'){
				error += 'Atleast one lead should be Won for Sales Create Approval\n';
				}				
			if(arr[2].trim() == 'no'){
				error = 'Lead Expected Clouser date is less then current date Please update';
				}
		...
		xmlhttp.open("GET","get_salesapproval_database.php?p_record="+str,false);
		xmlhttp.send(false);
...
//Add by Raghvender Singh on 01052014
function showApprovalinformationDatabase(str)
{
		...
		xmlhttp.onreadystatechange=function()
		  ...
		  if (xmlhttp.readyState==4 && xmlhttp.status==200)
			...
			if(arr[0].trim() == 'no'){
				error += 'Total Package Sold Amount and Total Payment Amount is not Match"\n';
				}
			if(arr[1].trim() == 'no'){
				error += 'Please attach your document\n';
				}				
			if(arr[2].trim() == 'no'){
				error = 'TDS Amount is greater than total package sold amount';
				}
		...
		xmlhttp.open("GET","get_approvalinformation_database.php?approval_id="+str,false);
		xmlhttp.send(false);
