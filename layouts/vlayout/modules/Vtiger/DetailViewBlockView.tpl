...
{strip}
	...
	{assign var=WIDTHTYPE value=$USER_MODEL->get('rowheight')}
        ...
        {if $MODULE eq 'Contacts' || $MODULE eq 'Events' || $MODULE eq 'Calendar' || $MODULE eq 'Potentials'}
	...
	<table class="table table-bordered equalSplit detailview-table">
        ...
        <tr>
            <td colspan="4">
                {if $MODULE_NAME eq 'Events' && vtranslate({$BLOCK_LABEL_KEY},{$MODULE_NAME}) eq 'Related Lead' && $ENTITYID ne ''}
		...
		{foreach item=FIELD_MODEL key=FIELD_NAME from=$FIELD_MODEL_LIST}
			...
			{else}
				...
				{else}
                 ...
                 {*<!-- Start to hide the level of Events module by jitendra singh on 28 March 14-->*}
                 {if ($MODULE eq 'Calendar' || $MODULE eq 'Events') && $FIELD_MODEL->get('label') eq 'Contact Name' || $FIELD_MODEL->get('label') eq 'Related To' && $contact_id neq ''}
                 &nbsp;
                 {else}
