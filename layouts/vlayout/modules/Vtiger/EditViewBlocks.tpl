...
{literal}
    <script type="text/javascript">
        ...
        function getOfferedAmount(entityid, entityvalue, product){
            checkFloatNumber(entityid);
            var rowid = entityid.split("offered_amount_"+product)[1];
            var bottom_price =  $("#bottom_price_"+product+rowid).val().replace(/,/g, '');
            var offered_amount = $("#offered_amount_"+product+rowid).val().replace(/,/g, '');
        ...
        function getNewMRPBottomPrice(entityid, entityvalue, product){
            checkIntegerNumber(entityid);
            var rowid = entityid.split("__")[1];
            var previousonevalue = '';
            var previousvalue =  $("#product_type_"+product+"___"+rowid).val().replace(/,/g, '');
            if(product == 'events') {
                previousonevalue =  $("#sponsorship_"+product+"___"+rowid).val().replace(/,/g, '');
                previousonevalue = previousonevalue.split("__")[0];
            }
            if(product == 'inventory') {
                previousonevalue =  $("#active_"+product+"___"+rowid).val().replace(/,/g, '');
            }
        ...
        function checkFloatNumber(entityid){
        ...
        function checkIntegerNumber(entityid){
            $('#'+entityid).keyup(function() {
                //$('span.error-keyup-1').hide();
                var inputVal = $(this).val();
                //alert(field_id1+'___'+inputVal);
...
<form class="form-horizontal recordEditView" id="EditView" name="EditView" method="post" action="index.php" enctype="multipart/form-data">
    ...
    <div class="contentHeader row-fluid">
        ...
        {/if}
        {* start customer data list*}
        {if $MODULE eq 'Contacts'}
            <th><strong style="color:red">Find Duplicate Client Name : </strong></th>
            <td class="fieldValue medium">
                <span class="add-on relatedPopup cursorPointer">
                    <i title="Select" class="icon-search relatedPopup" id="SalesOrder_editView_fieldName_contact_id_select"></i>
                </span>
            </td>
        {/if}
        {* end customer data list*}
    ...
    {*<!-- Start Code to hide Post Call Updates Tab by jitendra singh on 25 March 2014 -->*}
        ...
        {*<!-- This code is added for more contact when first time then it will be shown otherwise it will not shown -->*}

        {else if $MODULE eq 'Contacts' && $MODE neq '' && $BLOCK_LABEL eq "Contact Information"}
        ...
        {*<!-- Start Code to hide conveynence Tab when "Meeting" activity type is selected by jitendra singh on 24 March 2014 -->*}
            ...
            {else if $MODULE eq 'Events' && $BLOCK_LABEL eq "Related Lead"}
            
            	 {*<!--Change by Raghvender Singh on 03052014-->*}       
            	<div class="related_lead_block">
     {*<!--End Change by Raghvender Singh on 03052014-->*}  
...
{* start code added by ajay *}
    {if $APPROVAL_EDIT_PERMISSION eq 1}
        <tr>
            <td colspan="4">
                {if $MODULE eq 'Potentials'}
                    {foreach item=data key=PICKLIST_NAME from=$UNIQUE_PRODUCTS}
                        ...
                        <br>
                    {/foreach}
                    {include file="LineItemsSMARTJOBS.tpl"|@vtemplate_path:$MODULE}
...
{* start code added by jitendra singh for multiple document on 1 May 2014 *}

{if $MODULE eq 'Documents' && $MODE eq ''}

 {include file="LineItemsDocuments.tpl"|@vtemplate_path:$MODULE}
 
 {/if}
 
 
 {* End code added by jitendra singh for multiple document on 1 May 2014 *}
