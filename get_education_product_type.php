...
global $adb;
$sponser_qry = "select distinct(education_product) as education_product1 from vtiger_vendor 
	...
	echo "<select name='education_product' id='education_product' onchange='showEducationPrices(this.value);' >";
	echo "<option>Select an Option</option>";
	while($row = $adb->fetch_array($query)) {	
	$education_product = $row['education_product1'];
	echo "<option>$education_product</option>"; 
	}
	echo "</select>";
?> 